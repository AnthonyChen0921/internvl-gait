...
def _pool_compact_and_text(compact_tokens, text_embeds, attention_mask):
    ...
    text_mask = attention_mask.to(dtype=compact_tokens.dtype).unsqueeze(-1)
    text_embeds = text_embeds * text_mask
    text_len = text_mask.sum(dim=1).clamp(min=1.0)
    text_pooled = text_embeds.sum(dim=1) / text_len
    compact_pooled = compact_tokens.mean(dim=1)
...
def evaluate(decoder, language_model, classifier, data_loader, tokenizer, device):
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Evaluating", leave=False):
            ...
            compact_tokens = decoder(pixel_values=images)
...
def main():
    if not os.path.exists(STATE_PATH):
        raise FileNotFoundError(
            f"{STATE_PATH} not found. Run train_evl_compact_skeltext_no_llm_classifier.py first."
        )
