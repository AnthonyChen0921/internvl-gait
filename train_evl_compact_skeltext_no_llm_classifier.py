...
CKPT_PATH = "best_evl_compact_skeltext_no_llm_classifier.pt"
STATE_PATH = "evl_compact_skeltext_no_llm_train_state.pt"
...
def _pool_compact_and_text(compact_tokens, text_embeds, attention_mask):
    """
    Pool over [compact_tokens + text_embeds] without an LLM.
    Returns a single [B, D] feature.
    """
    B, M, D = compact_tokens.shape
    text_mask = attention_mask.to(dtype=compact_tokens.dtype)
    text_mask = text_mask.unsqueeze(-1)  # [B, L, 1]

    text_embeds = text_embeds * text_mask
    text_len = text_mask.sum(dim=1).clamp(min=1.0)
    text_pooled = text_embeds.sum(dim=1) / text_len  # [B, D]

    compact_pooled = compact_tokens.mean(dim=1)  # [B, D]
    return (compact_pooled + text_pooled) * 0.5
...
def evaluate(decoder, language_model, classifier, data_loader, tokenizer, device):
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Evaluating", leave=False):
            ...
            feats = _pool_compact_and_text(compact_tokens, text_embeds, attention_mask).float()
...
def main():
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        if macro_f1 > best_macro_f1:
            ...
            print(f"Saved new best EVL-compact+skeltext (no LLM) model (macro-F1={macro_f1*100:.2f}%)")
