...
CKPT_PATH = "best_evl_dualstream_skeltext_classifier.pt"
STATE_PATH = "evl_dualstream_skeltext_train_state.pt"
...
def collate_fn(batch, tokenizer, device):
    ...
    We keep images + labels and attach a prompt with aligned skeleton text.
    """
...
def _extract_qm_feats(decoder, images):
    qm_tokens = decoder(pixel_values=images)  # [B, M, D]
    feats_qm = qm_tokens.mean(dim=1).float()
    return feats_qm


def evaluate(img_model, decoder, classifier, data_loader, tokenizer, device):
    img_model.eval()
    decoder.eval()
    classifier.eval()
    correct = 0
...
def main():
    ...
    num_classes = len(LABELS)
    classifier = nn.Linear(hidden_size * 2, num_classes, dtype=torch.float32).to(DEVICE)
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        _, macro_f1 = evaluate(img_model, decoder, classifier, test_loader, tokenizer, DEVICE)
        if macro_f1 > best_macro_f1:
            ...
            print(f"Saved new best EVL dual-stream+skeltext model (macro-F1={macro_f1*100:.2f}%)")
